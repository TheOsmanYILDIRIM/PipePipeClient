...
object GeminiSubtitleHelper {
        ...
        bgExecutor.execute {
            try {
                ...
                if (cached.isNotEmpty()) {
                    ...
                    // Check if translation is partial (i.e. more chunks need to be translated)
                    val first = cached.first()
                    val totalChunks = first.totalChunks
                    if (cached.size < totalChunks && !streamInfo.subtitles.isNullOrEmpty()) {
                        // Find matching or default subtitle stream to resume background translation
    ...
    fun startLiveTicker(
        ...
        if (tickerRunnable != null) {
            // Ticker is already running, just updated activeBlocks
            return
        }
